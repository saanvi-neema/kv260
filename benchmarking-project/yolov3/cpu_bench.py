...
WARNING: YOLOv3 is very slow on ARM (~4.6s/frame). Keep N_BENCHMARK=10.
Let board cool between runs.
"""
import os
import time
import numpy as np
...
N_WARMUP = 2
N_BENCHMARK = 10  # ~46 seconds total — board runs hot

SCRIPT_DIR   = os.path.dirname(os.path.abspath(__file__))
MODEL_LOCAL  = os.path.join(SCRIPT_DIR, "models", "yolov3-10.onnx")
...
def run():
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        print("Run setup_all.sh first, or copy yolov3-10.onnx to models/")
        exit(1)

    print(f"Using model: {MODEL_PATH}", flush=True)
    print(f"Estimated time: ~{(N_WARMUP + N_BENCHMARK) * 4.6:.0f} seconds total", flush=True)
    print("Loading YOLOv3 ONNX model...", flush=True)
    session = ort.InferenceSession(MODEL_PATH, providers=['CPUExecutionProvider'])

    # YOLOv3 has two inputs: image + image_shape
    ...
    print(f"Inputs: {[i.name for i in session.get_inputs()]}", flush=True)
    ...
    print("="*40)
    print(f"Expected: ~0.22 FPS, ~6.25W, ~0.03 FPS/Watt")
