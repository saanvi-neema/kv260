...
WARNING: CPU runs hot — keep N_BENCHMARK=20. Let board cool between runs.
"""
import os
import time
import numpy as np
...
N_WARMUP = 3
N_BENCHMARK = 20  # keep low — CPU runs hot at 100% load

# Find model — check local models/ first, then /home/ubuntu/
SCRIPT_DIR   = os.path.dirname(os.path.abspath(__file__))
MODEL_LOCAL  = os.path.join(SCRIPT_DIR, "models", "resnet50-v1-7.onnx")
...
def run():
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        print("Run setup_all.sh first, or copy resnet50-v1-7.onnx to models/")
        exit(1)

    print(f"Using model: {MODEL_PATH}", flush=True)
    print("Loading ResNet50 ONNX model...", flush=True)
    ...
    print("="*40)
    print(f"Expected: ~1.59 FPS, ~4.35W, ~0.37 FPS/Watt")
